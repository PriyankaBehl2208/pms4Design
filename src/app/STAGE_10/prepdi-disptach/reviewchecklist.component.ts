...
@Component({
  selector: 'app-reviewchecklist',
   imports: [CommonModule,FormsModule],
  templateUrl: './reviewchecklist.component.html',
  styleUrl: './reviewchecklist.component.css'
})


export class REVIEWCHECKLISTComponent {
...
// videoStream: MediaStream | null = null;
 ...
 constructor( private ngZone: NgZone,private router:Router,private apiService:ServicesService,private http: HttpClient,private sessionService: SessionService){
...
FinalSAVE()
{
 ...
 debugger;
 this.apiService.FINAL_SAVE_REVIEW(this.FINAL_SAVE).subscribe({
...
this.GETMAIN_LIST.Pin=this.Pin;
  debugger;
    ...
    if(this.Department!="" && this.Engine_NO!="" && this.Model_NO!=""  && this.Pin!="")
    {
     ...
     this.apiService.GETMAIN_LIST_REVIEW(this.GETMAIN_LIST).subscribe({
...
this.http.post(
  ...
  'https://localhost:7294/api/PREPDI/UploadImage',
  formData,
  { headers }
).subscribe({
  next: () => {
...
statusok(id: number, event: Event) {
  event.preventDefault();
  const row = this.responseData.find(r => r.id === id);
  if (row) {
...
statusdone(id: number, event: Event) {
  event.preventDefault();
  const row = this.responseData.find(r => r.id === id);
  if (row) {
 ...
 STATUSOK()
 {
  ...
  this.REMARKS_VAL={}
this.REMARKS_VAL.Remarks_ID=this.remarkId.toString();
this.REMARKS_VAL.Remarks="OK";


 this.apiService.UPDATE_SATUSOK(this.REMARKS_VAL).subscribe({
...
statusnotok(id: number, event: Event) {
  event.preventDefault();
  const row = this.responseData.find(r => r.id === id);
  if (row) {

    debugger;
    row.prepdistatus = 'NOT OK';
    this.remarkId = id;
    this.STATUSNOTOK();
  }
}
 ...
 STATUSNOTOK()
 {
  ...
  this.REMARKS_VAL={}
this.REMARKS_VAL.Remarks_ID=this.remarkId.toString();
this.REMARKS_VAL.Remarks="NOT OK";
...
this.REMARKS_VAL.Remarks="OK";
 ...
 this.apiService.STATUSDONE_OK(this.REMARKS_VAL).subscribe({
